...
def _init():
    for k, v in {
        ...
        "upload_key":    0,        # increment to clear the uploader widget
    }.items():
...
with st.sidebar:
    ...
    # Helper: ingest (filename, bytes) pairs into session state
    def _ingest_pairs(pairs):
        if not pairs:
        ...
        try:
            ...
            for lbl, series in new_groups.items():
                if lbl not in st.session_state.series_map:
                    ...
                    added += 1
            if added:
                st.success(f"✓ {added} series loaded ({len(pairs)} files)")
    ...
    # ── Individual files ──────────────────────────────────────────────────────
    st.caption("**Option 1** — Select individual .dcm / .ima files")
    ...
    if st.button("Load selected files", use_container_width=True, type="primary",
                 key="load_files"):
        if not uploaded:
            st.warning("Select one or more files above first.")
        else:
            with st.spinner(f"Reading {len(uploaded)} file(s)…"):
                pairs = [(f.name, f.read()) for f in uploaded]
            ok = _ingest_pairs(pairs)
            if ok:
                st.session_state.upload_key += 1   # clears the uploader queue
                st.rerun()

    st.markdown("---")

    # ── ZIP folder ────────────────────────────────────────────────────────────
    st.caption("**Option 2** — Upload a .zip of your DICOM folder")
    st.markdown(
        "<small style='color:#888'>Zip your DICOM folder on Windows: "
        "right-click the folder → Send to → Compressed (zipped) folder.<br>"
        "On Mac: right-click → Compress.</small>",
        unsafe_allow_html=True,
    )
    ...
    if st.button("Load ZIP", use_container_width=True, type="primary",
                 key="load_zip"):
        if not zip_file:
            st.warning("Upload a .zip file above first.")
        else:
                        ...
                        for zname in z.namelist():
                            if not zname.endswith("/"):
                                pairs.append((os.path.basename(zname),
                                              z.read(zname)))
                    st.info(f"Found {len(pairs)} files in ZIP.")
                except Exception as e:
            ...
            if pairs:
                ok = _ingest_pairs(pairs)
                if ok:
                    st.session_state.upload_key += 1   # clears uploader queue
                    st.rerun()

    st.markdown("---")

    # ── Folder picker via webkitdirectory JS injection ────────────────────────
    st.caption("**Option 3** — Select a folder directly (Chrome / Edge / Firefox)")
    st.markdown(
        "<small style='color:#888'>Click below, then choose a folder. "
        "All DICOM files inside (including sub-folders) will be selected automatically.</small>",
        unsafe_allow_html=True,
    )

    # A normal multi-file uploader — JS below sets webkitdirectory on its input
    folder_uploaded = st.file_uploader(
        "folder",
        accept_multiple_files=True,
        label_visibility="collapsed",
        key=f"folder_uploader_{st.session_state.upload_key}",
    )

    # Inject JS once to set webkitdirectory on the third file input in the sidebar
    # (Option 1 = index 0, Option 2/zip = index 1, Option 3/folder = index 2)
    st.markdown("""
<script>
(function() {
  function patchFolderInput() {
    // Find all file inputs inside the sidebar
    var sidebar = document.querySelector('[data-testid="stSidebar"]');
    if (!sidebar) return;
    var inputs = sidebar.querySelectorAll('input[type="file"]');
    // The third file input (index 2) is our folder picker
    if (inputs.length >= 3) {
      var inp = inputs[2];
      ...
      setTimeout(patchFolderInput, 300);
    }
  }
  // Run on load and after any Streamlit re-render
  patchFolderInput();
  var observer = new MutationObserver(function() { patchFolderInput(); });
  observer.observe(document.body, { childList: true, subtree: true });
})();
</script>
""", unsafe_allow_html=True)

    if st.button("Load folder files", use_container_width=True, type="primary",
                 key="load_folder"):
        if not folder_uploaded:
            st.warning("Select a folder using the picker above, then click this button.")
        else:
            with st.spinner(f"Reading {len(folder_uploaded)} file(s) from folder…"):
                pairs = [(f.name, f.read()) for f in folder_uploaded]
            ok = _ingest_pairs(pairs)
            if ok:
