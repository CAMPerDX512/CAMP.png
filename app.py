...
with st.sidebar:
    ...
    tab_files, tab_folder, tab_zip = st.tabs(["Files", "Folder", "ZIP"])

    # Helper: process a list of (filename, bytes) pairs and load into session
    ...
    # ── Tab 1: individual file picker ─────────────────────────────────────────
    with tab_files:
        st.caption("Select one or more .dcm / .ima files.")
        ...
        if st.button("Load", key="load_files",
                     use_container_width=True, type="primary"):
            if not uploaded:
                st.warning("Select files above first.")
            else:
    ...
    # ── Tab 2: folder picker (webkitdirectory) ────────────────────────────────
    with tab_folder:
        st.caption("Pick a folder — all DICOM files inside are loaded automatically, "
                   "including nested sub-folders.")

        # Custom HTML component using webkitdirectory
        # Files are read as base64 in the browser and sent back via
...
<style>
  ...
  #folder-btn {
    display:inline-block; background:#1e3a5f; color:#7ab8ff;
    border:1px solid #2a5080; border-radius:6px; padding:8px 16px;
    font-size:12px; cursor:pointer; width:100%; text-align:center;
    box-sizing:border-box;
  }
  #folder-btn:hover { background:#264f8a; }
  #status { margin-top:8px; font-size:11px; color:#aaa; min-height:16px; }
  #folder-input { display:none; }
</style>
<input type="file" id="folder-input" webkitdirectory multiple/>
<div id="folder-btn" onclick="document.getElementById('folder-input').click()">
  📁 Select Folder
</div>
...
document.getElementById('folder-input').addEventListener('change', function(e) {
  ...
  const dicomExts = new Set(['.dcm', '.ima', '']);
  const dicomFiles = files.filter(f => {
    const ext = f.name.includes('.') ? '.' + f.name.split('.').pop().toLowerCase() : '';
    return dicomExts.has(ext);
  });

  if (dicomFiles.length === 0) {
    document.getElementById('status').textContent =
      'No DICOM files found in folder. Try .dcm or .ima files.';
    return;
  }

  document.getElementById('status').textContent =
    'Reading ' + dicomFiles.length + ' files…';

  let done = 0;
  const results = [];

  dicomFiles.forEach((file, idx) => {
    const reader = new FileReader();
    reader.onload = function(ev) {
      // Store as [relativePath, base64data]
      results[idx] = [
      ...
      document.getElementById('status').textContent =
        'Reading ' + done + ' / ' + dicomFiles.length + '…';
      if (done === dicomFiles.length) {
        document.getElementById('status').textContent =
          '✓ ' + done + ' files ready — click Load Folder below';
        // Send to Streamlit
...
"""
        folder_data = components.html(
            folder_component_html,
            height=90,
            scrolling=False,
        )

        if st.button("Load Folder", key="load_folder",
                     use_container_width=True, type="primary"):
            if not folder_data:
                st.warning("Select a folder using the button above first, "
                           "then click Load Folder.")
            else:
                import base64
                with st.spinner("Decoding files…"):
                    pairs = []
                    for rel_path, b64 in folder_data:
                        try:
                            raw = base64.b64decode(b64)
                            pairs.append((os.path.basename(rel_path), raw))
                        except Exception:
                            pass
                ok = _ingest_pairs(pairs)
                if ok:
    ...
    # ── Tab 3: ZIP upload ─────────────────────────────────────────────────────
    with tab_zip:
        st.caption("Upload a .zip of your DICOM folder. "
                   "Nested sub-folders are extracted automatically.")
        ...
        if st.button("Load ZIP", key="load_zip",
                     use_container_width=True, type="primary"):
            if not zip_file:
